...
def ctdet_post_process(dets_act, c, s, h, w, num_obj_classes, num_act_classes, gt_vector):
  ret_act = []
  for i in range(dets_act.shape[0]):
    ...
    # if(dets_act[i, :, 8] > 0): x_flag = 1
    # if(dets_act[i, :, 8] < 0): x_flag = 0
    # if(dets_act[i, :, 9] > 0): y_flag = 1
    # if(dets_act[i, :, 9] < 0): y_flag = 0


    # print(dets_act[0])

    classes_act = dets_act[i, :, -3]

    for j in range(num_act_classes):
      inds = (classes_act == j)
      top_preds_act[j + 1] = np.concatenate([
        dets_act[i, inds, :6].astype(np.float32),
        dets_act[i, inds, 6:7].astype(np.float32),
        dets_act[i, inds, 8:10].astype(np.float32)], axis=1).tolist()

    ret_act.append(top_preds_act)
    # ret_act.append(x_flag)
    # ret_act.append(y_flag)
